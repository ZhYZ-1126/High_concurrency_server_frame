...
static uint64_t s_http_request_buffer_size = 0;     //请求头部一个字段的大小
static uint64_t s_http_request_max_body_size = 0;   //请求消息体一个字段的大小
static uint64_t s_http_response_buffer_size = 0;    //响应头部一个字段的大小
static uint64_t s_http_response_max_body_size = 0;  //响应消息体一个字段的大小
...
void on_request_method(void *data,const char *at,size_t length)
{
    ...
    if(m == HttpMethod::INVALID_METHOD) {//HTTP请求方法枚举类不存在m
        HCSERVER_LOG_WARN(g_logger) << "invalid http request method: "<< string(at, length);
...
size_t HttpRequestParser::execute(char* data, size_t len)
{
    ...
    //从data的offset位置开始，移除剩下的还未解析的长度(len - offset)的值到data上，从data的第一个元素开始存放----将data上已经解析的数据移除
